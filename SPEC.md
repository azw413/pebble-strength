...
| Exercise library | ~30 built-in movements seeded into the server DB with body area, muscle groups worked, and a rep-counting profile; the watch's table is generated from the same seed; "Custom" movement with generic profile. Users can add their own movements on the Exercises page — these are private, get a `watch_movement_id` from the reserved 128–255 range (per user, so each account has all 128 slots), and use the generic profile until tuned |
